...
exports.deleteCompleteMessageData = functions.firestore.document('/messages/{messageId}')
    .onDelete(async(snap, context) => {
        const { messageId } = context.params;
        const bucket = firebase.storage().bucket();
        return bucket.deleteFiles({
            prefix: `+00_CHAT_MEDIA/${messageId}`
        });
    });
exports.deleteMessageChatDocs = functions.firestore.document('/messages/{messageId}')
    .onDelete(async(snap, context) => {
        const { messageId } = context.params;
        return db.collection(`messages/${messageId}/${messageId}`)
            .get()
            .then(res => {
        ...
        if (isMultilangNotificationEnabled == true) {
            ...
            payload = {
                ...
                data: {
                    ...
                    'bodyMultilang': 'New message(s) received.',
                    'title': 'You have new message(s)',
                    'body': 'New message(s) received.',
                    'peerid': message.from,
                    'click_action': 'FLUTTER_NOTIFICATION_CLICK',
...
exports.createCallsWithTokens = functions.https.onCall(async (data,context)=>{
  ...
  return Math.floor(Math.random() * max);
        ...
        const uid = 0
        const channelName = getRandomInt(1000000000000000).toString()
